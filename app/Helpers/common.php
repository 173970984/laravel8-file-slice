...
if (!function_exists('checkSet')) {
     ...
     * @date 2019/5/23
     */
    function checkSet($field, $default = '')
    {
        $data = request()->all();
        $result = $default;
        if (isset($data[$field])) {
...
if (!function_exists('upload_file')) {
    ...
    function upload_file($request, $form_name = 'file')
    {
        ...
        // 文件大小校验(MAX=5M)
        $file_max_size = 5000000 * 1024 * 1024;
        if ($size > $file_max_size) {
            return message("您上传的文件过大,最大值为" . $file_max_size / 1024 / 1024 . "MB");
        }

        // 允许上传的文件后缀
        $file_exts = array('xls', 'xlsx', 'csv',"mp4","flv","m3u8");
        if (!in_array(strtolower($ext), $file_exts)) {
